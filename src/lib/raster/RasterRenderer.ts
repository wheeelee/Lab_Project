...
export class RasterRenderer {
  ...
  resize(): void {
    ...
    // Keep previously drawn content visible after resize by fitting old pixels
    // into the new buffer with nearest-neighbor scaling.
    if (oldBuf && oldWidth > 0 && oldHeight > 0) {
      for (let y = 0; y < height; y++) {
        const srcY = Math.min(oldHeight - 1, Math.floor((y * oldHeight) / height));
        for (let x = 0; x < width; x++) {
          const srcX = Math.min(oldWidth - 1, Math.floor((x * oldWidth) / width));
          const srcIdx = (srcY * oldWidth + srcX) * 4;
          const dstIdx = (y * width + x) * 4;
          this.buf[dstIdx] = oldBuf[srcIdx];
          this.buf[dstIdx + 1] = oldBuf[srcIdx + 1];
          this.buf[dstIdx + 2] = oldBuf[srcIdx + 2];
          this.buf[dstIdx + 3] = oldBuf[srcIdx + 3];
        }
      }
    }
  }
  ...
  private drawHSpan(y: number, x0: number, x1: number, color: RGBA): void {
    ...
    for (let x = start; x <= end; x++) this.setPixel(x, y, color);
  }

  fillPolygon(points: { x: number; y: number }[], color: RGBA): void {
  if (points.length < 3) return;
  const minY = Math.max(0, Math.floor(Math.min(...points.map(p => p.y))));
  const maxY = Math.min(this.height - 1, Math.floor(Math.max(...points.map(p => p.y))));
  ...
  for (let y = minY; y <= maxY; y++) {
    ...
    for (let i = 0; i < intersections.length; i += 2) {
      let xStart = Math.floor(intersections[i]);
      let xEnd = Math.floor(intersections[i + 1]);

      // Ограничение по горизонтали
      xStart = Math.max(0, Math.min(this.width - 1, xStart));
      xEnd = Math.max(0, Math.min(this.width - 1, xEnd));
  ...
  fillCircle(cx: number, cy: number, radius: number, color: RGBA): void {
    ...
    for (let y = -radius; y <= radius; y++) {
      ...
      const xStart = Math.max(0, cx - dx);
      const xEnd = Math.min(this.width - 1, cx + dx);
